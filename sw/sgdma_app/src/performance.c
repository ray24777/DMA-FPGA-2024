/*
 * This file is part of the Anlogic DMA IP Core driver tools for Linux
 *
 * Copyright (c) 2016-present,  Anlogic, Inc.
 * All rights reserved.
 *
...
#include "../../sgdma_drv/cdev_sgdma.h"

struct sgdma_performance_ioctl perf;
...
static void usage(const char* name)
{
  ...
  printf("Performance test for SGDMA SGDMA engine.\n\n");
...
{
  int cmd_opt;
  char *device = "/dev/ANLOGIC-PCI0_h2c_0";
  uint32_t size = 32768;
  uint32_t count = 1;
  char *filename = NULL;
...
int test_dma(char *device_name, int size, int count)
{
  ...
  if (fd < 0) {
      printf("FAILURE: Could not open %s. Make sure sgdma device driver is loaded and you have access rights (maybe use sudo?).\n", device_name);
      exit(1);
  }

  unsigned char status = 1;

  perf.version = IOCTL_SGDMA_PERF_V1;
  perf.transfer_size = size;
  rc = ioctl(fd, IOCTL_SGDMA_PERF_START, &perf);
  if (rc == 0) {
    printf("IOCTL_SGDMA_PERF_START succesful.\n");
  } else {
    printf("ioctl(..., IOCTL_SGDMA_PERF_START) = %d\n", rc);
  }
#if 1
  while (count--) {

    sleep(2);
    rc = ioctl(fd, IOCTL_SGDMA_PERF_GET, &perf);
    if (rc == 0) {
      printf("IOCTL_SGDMA_PERF_GET succesful.\n");
    } else {
      printf("ioctl(..., IOCTL_SGDMA_PERF_GET) = %d\n", rc);
    }
...
#endif
  rc = ioctl(fd, IOCTL_SGDMA_PERF_STOP, &perf);
  if (rc == 0) {
    printf("IOCTL_SGDMA_PERF_STOP succesful.\n");
  } else {
    printf("ioctl(..., IOCTL_SGDMA_PERF_STOP) = %d\n", rc);
  }
  ...
  if (perf.clock_cycle_count && perf.data_cycle_count) {
    printf("(data duty cycle = %lld%%)\n", (long long)perf.data_cycle_count * 100 / (long long)perf.clock_cycle_count);
    printf (" data rate ***** bytes length = %d, rate = %f \n", perf.transfer_size, (double)(long long)perf.data_cycle_count*2/(long long)perf.clock_cycle_count);
  }
