...
#ifndef ANLOGIC_PCIE_LIB_H
#define ANLOGIC_PCIE_LIB_H
...
#define SGDMA_BAR_NUM (6)

/* anlogic PCIe device specific book-keeping */
#define XDEV_FLAG_OFFLINE   0x1

#define MAX_USER_IRQ 16

#define SGDMA_OFS_INT_CTRL   (0x2000UL)
#define SGDMA_OFS_CONFIG     (0x3000UL)

/* maximum number of desc per transfer request */
#define SGDMA_ENGINE_XFER_MAX_DESC       0x800
#define SGDMA_ENGINE_CREDIT_XFER_MAX_DESC    0x3FF

//#define __LIBSGDMA_DEBUG__
 ...
 *  all of the bars
 */
//#define SGDMA_CONFIG_BAR_NUM   1

/* Define anlogic user bar and base offset */
#define SGDMA_USER_BAR_NUM 0

#define SGDMA_USER_BAR_BASE_OFFSET (0x80000)

/* Switch debug printing on/off */
#define SGDMA_DEBUG 0

/* SECTION: Preprocessor macros/constants */
#define SGDMA_BAR_NUM (6)

/* maximum amount of register space to map */
#define SGDMA_BAR_SIZE (0x8000UL)

/* Use this definition to poll several times between calls to schedule */
#define NUM_POLLS_PER_SCHED 100

#define SGDMA_CHANNEL_NUM_MAX (1)
/*
 * interrupts per engine, rad2_vul.sv:237
 * .REG_IRQ_OUT (reg_irq_from_ch[(channel*2) +: 2]),
 */
#define SGDMA_ENG_IRQ_NUM (1)
#define MAX_EXTRA_ADJ   0x3F 

#define SGDMA_MAX_ADJ_BLOCK_SIZE 0x40
#define SGDMA_PAGE_SIZE          0x1000

#define RX_STATUS_EOP (1)

/* Target internal components on SGDMA control BAR */
#define SGDMA_OFS_INT_CTRL   (0x2000UL)
#define SGDMA_OFS_CONFIG     (0x3000UL)

/* maximum number of desc per transfer request */
#define SGDMA_TRANSFER_MAX_DESC (2048)

/* maximum size of a single DMA transfer descriptor */
#define SGDMA_DESC_BLEN_BITS 12
#define SGDMA_DESC_BLEN_MAX  ((1UL << (SGDMA_DESC_BLEN_BITS))) /* xxx hwb */

/* bits of the SG DMA control register */
#define SGDMA_CTRL_RUN_STOP          (1UL << 0)
#define SGDMA_CTRL_IE_DESC_STOPPED       (1UL << 1)
#define SGDMA_CTRL_IE_DESC_COMPLETED     (1UL << 2)
#define SGDMA_CTRL_IE_DESC_ALIGN_MISMATCH    (1UL << 3)
#define SGDMA_CTRL_IE_MAGIC_STOPPED      (1UL << 4)
#define SGDMA_CTRL_IE_IDLE_STOPPED       (1UL << 6)
#define SGDMA_CTRL_IE_READ_ERROR         (0x1FUL << 9)
#define SGDMA_CTRL_IE_DESC_ERROR         (0x1FUL << 19)
#define SGDMA_CTRL_NON_INCR_ADDR         (1UL << 25)
#define SGDMA_CTRL_POLL_MODE_WB          (1UL << 26)
#define SGDMA_CTRL_STM_MODE_WB           (1UL << 27)

/* bits of the SG DMA status register */
#define SGDMA_STAT_BUSY          (1UL << 0)
#define SGDMA_STAT_DESC_STOPPED      (1UL << 1)
#define SGDMA_STAT_DESC_COMPLETED    (1UL << 2)
#define SGDMA_STAT_ALIGN_MISMATCH    (1UL << 3)
#define SGDMA_STAT_MAGIC_STOPPED     (1UL << 4)
#define SGDMA_STAT_INVALID_LEN       (1UL << 5)
#define SGDMA_STAT_IDLE_STOPPED      (1UL << 6)

#define SGDMA_STAT_COMMON_ERR_MASK \
    (SGDMA_STAT_ALIGN_MISMATCH | SGDMA_STAT_MAGIC_STOPPED | \
     SGDMA_STAT_INVALID_LEN)

/* desc_error, C2H & H2C */
#define SGDMA_STAT_DESC_UNSUPP_REQ   (1UL << 19)
#define SGDMA_STAT_DESC_COMPL_ABORT  (1UL << 20)
#define SGDMA_STAT_DESC_PARITY_ERR   (1UL << 21)
#define SGDMA_STAT_DESC_HEADER_EP    (1UL << 22)
#define SGDMA_STAT_DESC_UNEXP_COMPL  (1UL << 23)

#define SGDMA_STAT_DESC_ERR_MASK \
    (SGDMA_STAT_DESC_UNSUPP_REQ | SGDMA_STAT_DESC_COMPL_ABORT | \
     SGDMA_STAT_DESC_PARITY_ERR | SGDMA_STAT_DESC_HEADER_EP | \
     SGDMA_STAT_DESC_UNEXP_COMPL)

/* read error: H2C */
#define SGDMA_STAT_H2C_R_UNSUPP_REQ  (1UL << 9)
#define SGDMA_STAT_H2C_R_COMPL_ABORT (1UL << 10)
#define SGDMA_STAT_H2C_R_PARITY_ERR  (1UL << 11)
#define SGDMA_STAT_H2C_R_HEADER_EP   (1UL << 12)
#define SGDMA_STAT_H2C_R_UNEXP_COMPL (1UL << 13)

#define SGDMA_STAT_H2C_R_ERR_MASK    \
    (SGDMA_STAT_H2C_R_UNSUPP_REQ | SGDMA_STAT_H2C_R_COMPL_ABORT | \
     SGDMA_STAT_H2C_R_PARITY_ERR | SGDMA_STAT_H2C_R_HEADER_EP | \
     SGDMA_STAT_H2C_R_UNEXP_COMPL)

/* write error, H2C only */
#define SGDMA_STAT_H2C_W_DECODE_ERR  (1UL << 14)
#define SGDMA_STAT_H2C_W_SLAVE_ERR   (1UL << 15)

#define SGDMA_STAT_H2C_W_ERR_MASK    \
    (SGDMA_STAT_H2C_W_DECODE_ERR | SGDMA_STAT_H2C_W_SLAVE_ERR)

/* read error: C2H */
#define SGDMA_STAT_C2H_R_DECODE_ERR  (1UL << 9)
#define SGDMA_STAT_C2H_R_SLAVE_ERR   (1UL << 10)

#define SGDMA_STAT_C2H_R_ERR_MASK    \
    (SGDMA_STAT_C2H_R_DECODE_ERR | SGDMA_STAT_C2H_R_SLAVE_ERR)

/* all combined */
#define SGDMA_STAT_H2C_ERR_MASK  \
    (SGDMA_STAT_COMMON_ERR_MASK | SGDMA_STAT_DESC_ERR_MASK | \
     SGDMA_STAT_H2C_R_ERR_MASK | SGDMA_STAT_H2C_W_ERR_MASK)

#define SGDMA_STAT_C2H_ERR_MASK  \
    (SGDMA_STAT_COMMON_ERR_MASK | SGDMA_STAT_DESC_ERR_MASK | \
     SGDMA_STAT_C2H_R_ERR_MASK)

/* bits of the SGDMA descriptor control field */
#define SGDMA_DESC_STOPPED   (1UL << 0)
#define SGDMA_DESC_COMPLETED (1UL << 1)
#define SGDMA_DESC_EOP       (1UL << 4)

#define SGDMA_PERF_RUN   (1UL << 0)
#define SGDMA_PERF_CLEAR (1UL << 1)
#define SGDMA_PERF_AUTO  (1UL << 2)

#define MAGIC_ENGINE    0xEEEEEEEEUL
#define MAGIC_DEVICE    0xDDDDDDDDUL

/* upper 16-bits of engine identifier register */
#define SGDMA_ID_H2C 0x1fc0U
#define SGDMA_ID_C2H 0x1fc1U
...
#define MAX_NUM_ENGINES (SGDMA_CHANNEL_NUM_MAX * 2)
...
#if KERNEL_VERSION(5, 16, 0) <= LINUX_VERSION_CODE
#define al_wake_up swake_up
#define al_wait_event_interruptible_timeout swait_event_interruptible_timeout
#define al_wait_event_interruptible \
            swait_event_interruptible
#else
#define al_wake_up wake_up_interruptible
/* wait_event_interruptible_timeout() could return prematurely (-ERESTARTSYS)
 * if it is interrupted by a signal */
#define al_wait_event_interruptible_timeout(wq, condition, timeout) \
({\
    int __ret = 0;  \
...
struct sgdma_request_cb {
    ...
    struct sgdma_transfer tfer[2]; /* Use two transfers in case single request needs to be split */
    struct sgdma_io_cb *cb;
...
struct anlogic_dma_engine {
    unsigned long magic;    /* structure ID for sanity checks */
    struct anlogic_dev *sgdev;  /* parent device */
    ...
    int eop_flush;     /* st c2h only, flush up the data with eop */
#else
    ...
    struct sgdma_result *cyclic_result;
    dma_addr_t cyclic_result_bus;   /* bus addr for transfer */
    struct sgdma_request_cb *cyclic_req;
    ...
    /* used for tx bd ring:h2c */
    int tx_tail;    /* follows the HW */
    int tx_head;    /* where the SW reads from */
    int tx_overrun; /* flag if overrun occured */
    int tx_desc_cnt;
    int tx_ring_desc_num;
    ...
    struct sgdma_performance_ioctl *sgdma_perf;   /* perf test control */
#if KERNEL_VERSION(5, 16, 0) <= LINUX_VERSION_CODE
    struct swait_queue_head sgdma_perf_wq;
#else
    wait_queue_head_t sgdma_perf_wq; /* Perf test sync */
#endif

    struct sgdma_kthread *cmplthp;
...
#ifdef AL_BD_RING
    /* al dma bd ring */
    struct al_ring *rx_ring;
    struct al_ring *tx_ring;
#endif
};

struct anlogic_user_irq {
    struct anlogic_dev *sgdev;       /* parent device */
    u8 user_idx;            /* 0 ~ 15 */
...
struct anlogic_dev {
    ...
    /* PCIe BAR management */
    void __iomem *bar[SGDMA_BAR_NUM];    /* addresses for mapped BARs */
    int user_bar_idx;   /* BAR index of user logic */
    int user_bar_base_offset;   /* BAR base offset of user logic */
    int config_bar_idx; /* BAR index of SGDMA config logic */
    int bypass_bar_idx; /* BAR index of SGDMA bypass logic */
...
#endif
    struct anlogic_user_irq user_irq[16];   /* user IRQ management */
    unsigned int mask_irq_user;

    /* SGDMA engine management */
    int engines_num;    /* Total engine count */
    u32 mask_irq_h2c;
    u32 mask_irq_c2h;
    struct anlogic_dma_engine engine_h2c[SGDMA_CHANNEL_NUM_MAX]; //engine control struct is static 
    struct anlogic_dma_engine engine_c2h[SGDMA_CHANNEL_NUM_MAX]; //engine control struct is static 
...
#ifdef __REG_DEBUG__
#define write_register(v, iomem, off) \
    do { \
        pr_info("[reg dbg w] %s(%d): w reg 0x%p(0x%lx), 0x%08x.\n", __func__, __LINE__, iomem, (unsigned long)(off), (u32)v); \
        iowrite32(v, iomem); \
    } while(0)

#define read_register(iomem) \
    ({ \
        u32 val; \
        val = ioread32(iomem); \
        pr_info("[reg dbg r] %s(%d): 0x%pK, R reg 0x%08x.\n", __func__, __LINE__, iomem, val); \
        val; \
    })
#else
#define write_register(v, iomem, off) iowrite32(v, iomem)

#define read_register(iomem) ioread32(iomem)
#endif

static inline int sgdma_device_flag_check(struct anlogic_dev *sgdev, unsigned int f)
{
    unsigned long flags;
...
int engine_service_poll(struct anlogic_dma_engine *engine, u32 expected_desc_count);
